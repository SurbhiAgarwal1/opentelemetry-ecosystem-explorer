...
[latest release](https://central.sonatype.com/artifact/io.opentelemetry.instrumentation/opentelemetry-log4j-appender-2.17).
...
In this example Log4j2 log events will be sent to both the console appender and the
...
The `otel.event.name` key is supported in `MapMessage` entries and context data entries. When
present, its value is used as the log event name and is not emitted as an attribute.
