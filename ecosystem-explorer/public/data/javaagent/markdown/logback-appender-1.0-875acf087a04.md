...
[latest release](https://central.sonatype.com/artifact/io.opentelemetry.instrumentation/opentelemetry-logback-appender-1.0).
...
In this example Logback log events will be sent to both the console appender and the
...
The `otel.event.name` key is supported in key-value pairs (SLF4J 2.x fluent API), MDC entries,
Logstash markers (e.g., `Markers.append("otel.event.name", ...)`), and Logstash structured arguments
(e.g., `StructuredArguments.keyValue("otel.event.name", ...)`). When present, its value is used as
the log event name and is not emitted as an attribute.
