...
[latest release](https://central.sonatype.com/artifact/io.opentelemetry.instrumentation/opentelemetry-logback-mdc-1.0).
...
> It's important to note you can also use other encoders in the `ConsoleAppender` like
> [logstash-logback-encoder](https://github.com/logfellow/logstash-logback-encoder). This can be
> helpful when the `Span` is invalid and the `trace_id`, `span_id`, and `trace_flags` are all `null`
...
If you set `<addBaggage>true</addBaggage>` in your `logback.xml` configuration, key/value pairs in
[baggage](https://opentelemetry.io/docs/concepts/signals/baggage/) will also be added to the MDC.
